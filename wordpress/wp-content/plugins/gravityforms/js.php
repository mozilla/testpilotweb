...
function ToggleConditionalLogic(isInit){
    var speed = isInit ? "" : "slow";
    if(jQuery('#field_conditional_logic').is(":checked")){

        CreateConditionalLogic(GetSelectedField());

        //Initializing object so it has the default options set
        SetConditionalProperty("actionType", jQuery("#gfield_action_type").val());
        SetConditionalProperty("logicType", jQuery("#gfield_logic_type").val());
        SetRule(0);

        jQuery('#field_conditional_logic_container').show(speed);
    }
    else{
        jQuery('#field_conditional_logic_container').hide(speed);
    }
...
function CreateConditionalLogic(field){
    if(!field.conditionalLogic)
        field.conditionalLogic = new ConditionalLogic();

    var hideSelected = field.conditionalLogic.actionType == "hide" ? "selected='selected'" :"";
    var showSelected = field.conditionalLogic.actionType == "show" ? "selected='selected'" :"";
    var allSelected = field.conditionalLogic.logicType == "all" ? "selected='selected'" :"";
    var anySelected = field.conditionalLogic.logicType == "any" ? "selected='selected'" :"";
    var imagesUrl = '<?php echo GFCommon::get_base_url() . "/images"?>';

    var field_str = "<select id='gfield_action_type' onchange='SetConditionalProperty(\"actionType\", jQuery(this).val());'><option value='show' " + showSelected + "><?php _e("Show", "gravityforms") ?></option><option value='hide' " + hideSelected + "><?php _e("Hide", "gravityforms") ?></option></select>";
    field_str += " <?php _e("this field if", "gravityforms") ?> ";
    field_str += "<select id='gfield_logic_type' onchange='SetConditionalProperty(\"logicType\", jQuery(this).val());'><option value='all' " + allSelected + "><?php _e("All", "gravityforms") ?></option><option value='any' " + anySelected + "><?php _e("Any", "gravityforms") ?></option></select>";
    field_str += " <?php _e("of the following match:", "gravityforms") ?> ";

    for(var i=0; i<field.conditionalLogic.rules.length; i++){
        var isSelected = field.conditionalLogic.rules[i].operator == "is" ? "selected='selected'" :"";
        var isNotSelected = field.conditionalLogic.rules[i].operator == "isnot" ? "selected='selected'" :"";

        field_str += "<div style='width:99%'>" + GetRuleFields(i, field.conditionalLogic.rules[i].fieldId);
        field_str += "<select id='rule_operator_" + i + "' onchange='SetRuleProperty(" + i + ", \"operator\", jQuery(this).val());'><option value='is' " + isSelected + "><?php _e("is", "gravityforms") ?></option><option value='isnot' " + isNotSelected + "><?php _e("is not", "gravityforms") ?></option></select>";
        field_str += GetRuleValues(i, field.conditionalLogic.rules[i].fieldId, field.conditionalLogic.rules[i].value);
        field_str += "<img src='" + imagesUrl + "/add.png' class='add_field_choice' title='add another rule' alt='add another rule' style='cursor:pointer; margin:0 3px;' onclick=\"InsertRule(" + (i+1) + ");\" />";
        if(field.conditionalLogic.rules.length > 1 )
            field_str += "<img src='" + imagesUrl + "/remove.png' title='remove this rule' alt='remove this rule' class='delete_field_choice' style='cursor:pointer;' onclick=\"DeleteRule(" + i + ");\" /></li>";

        field_str += "</div>";
    }

    jQuery("#field_conditional_logic_container").html(field_str);
}

function InsertRule(ruleIndex){
    field = GetSelectedField();
    field.conditionalLogic.rules.splice(ruleIndex, 0, new ConditionalRule());
    CreateConditionalLogic(field);
    SetRule(ruleIndex);
}

function SetRule(ruleIndex){
    SetRuleProperty(ruleIndex, "fieldId", jQuery("#rule_field_" + ruleIndex).val());
    SetRuleProperty(ruleIndex, "operator", jQuery("#rule_operator_" + ruleIndex).val());
    SetRuleProperty(ruleIndex, "value", jQuery("#rule_value_" + ruleIndex).val());
}
...
function GetRuleFields(ruleIndex, selectedFieldId){
    var str = "<select id='rule_field_" + ruleIndex + "' class='gfield_rule_select' onchange='jQuery(\"#rule_value_" + ruleIndex + "\").replaceWith(GetRuleValues(" + ruleIndex + ", jQuery(this).val())); SetRule(" + ruleIndex + "); '>";
    var inputType;
    for(var i=0; i<form.fields.length; i++){
        ...
        if(inputType == "checkbox" || inputType == "radio" || inputType == "select"){
            ...
            str += "<option value='" + form.fields[i].id + "' " + selected + ">" + TruncateRuleText(form.fields[i].label) + "</option>";
        }
    }
    str += "</select>";
    return str;
...
function GetRuleValues(ruleIndex, selectedFieldId, selectedValue){
    var str = "<select class='gfield_rule_select' id='rule_value_" + ruleIndex + "' onchange='SetRuleProperty(" + ruleIndex + ", \"value\", jQuery(this).val());'>";
    ...
    for(var i=0; i<field.choices.length; i++){
        var isSelected = field.choices[i].value == selectedValue;
        var selected = isSelected ? "selected='selected'" : "";
        if(isSelected)
            isAnySelected = true;

        str += "<option value='" + field.choices[i].value.replace("'", "&#039;") + "' " + selected + ">" + TruncateRuleText(field.choices[i].text) + "</option>";
    }
...
function SetRuleProperty(ruleIndex, name, value){
   var field = GetSelectedField();
   field.conditionalLogic.rules[ruleIndex][name] = value;
}

function SetConditionalProperty(name, value){
    var field = GetSelectedField();
    field.conditionalLogic[name] = value;
}
